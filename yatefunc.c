 ...
 *
 * $Id: yatefunc.c,v 1.1 2003/12/21 01:09:22 mclap Exp $
 */
#include <stdio.h>
#include <stdarg.h>
#include <stdlib.h>
		...
		case LNODE_OPER:
			switch (n->u.oper.id) {
				case ';':
					...
					v = nexec(ts,n->u.oper.args[0]);
					if (v) {
						vbuf_write(ts, v);
						if (v->tmp) vfree(v);
						v = 0;
					}
					r = nexec(ts,n->u.oper.args[1]);
					break;

				case IF:
					k = nexec(ts,n->u.oper.args[0]);
					if (val2bool(k)) {
						r = nexec(ts,n->u.oper.args[1]);
					}
					else {
						if (n->u.oper.count>2)
							r = nexec(ts,n->u.oper.args[2]);
					}
					break;
...
//						printf("WHILE_AFTER_EXPR (%08lx/%d)\n", k, k->tmp);
				...
				case '=':
					i = vfindn(ts->GLOBALS,n->u.oper.args[0]->u.data);
					v = nexec(ts,n->u.oper.args[1]);
					if (v && !v->tmp) v = vdupe(v);
						
					v->tmp = 0;
					if (i != (size_t)-1) {
//						printf("assign [%d]%08lx=%08lx", i, GLOBALS->u.aval.vals[i], v);
						VFREE(ts->GLOBALS->u.aval.vals[i]);
						ts->GLOBALS->u.aval.vals[i] = v;
...
int vbuf_write2(yate *ts, char *p) {
	unsigned int l;

	l = strlen(p);
	if (ts->out.len + l + 1 >= ts->out.size) {
		ts->out.size += l + (4096 - l&(4096-1));
		ts->out.buf = (char *)realloc(ts->out.buf, ts->out.size);
	}
	memcpy(ts->out.buf + ts->out.len, p, l);
	ts->out.len += l;
	ts->out.buf[ts->out.len] = 0;
	return l;
}
...
yate *yate_init() {
	...
	r = (yate *)calloc(1, sizeof(yate));
	if (!r) return 0;

	r->GLOBALS = vnew(LV_ARRAY);
